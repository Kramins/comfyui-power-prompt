...
// src/power-prompt-panel.js
import { app as app2 } from "../../scripts/app.js";

// src/constants.js
var YAML_MIN_HEIGHT = 220;
var CHIP_THRESHOLD = 20;
var OPTION_DISPLAY_MAX = 48;
var DEFAULT_YAML = `variables:
  subject:
    type: select
    count: 1
    options:
...
function getIncludeRawStrings(node) {
  const parts = [];
  const inputs = (node.inputs ?? []).filter((i) => i.name.startsWith("include_") && i.link != null).sort((a, b) => a.name.localeCompare(b.name));
...
function notifyConnectedMainNodes(partialNode) {
...
function createVarRow(ctrl, isLarge, savedValue) {
  ...
  if (ctrl.widget === "dropdown") {
    row.appendChild(isLarge ? createSearchableDropdown(ctrl.options, savedValue) : createDropdown(ctrl.options, savedValue));
  } else if (ctrl.widget === "checkboxes") {
    const savedArr = Array.isArray(savedValue) ? savedValue : [];
    row.appendChild(isLarge ? createChipSelect(ctrl.options, savedArr, ctrl.is_any) : createCheckboxGroup(ctrl.options, savedArr));
...
function createSearchableDropdown(options, savedValue) {
  ...
  input.addEventListener("keydown", (e) => {
    if (e.key === "Escape") {
      ...
      dropdown.querySelector(".pp-dropdown-item")?.dispatchEvent(new MouseEvent("mousedown"));
    }
  });
...
// src/power-prompt-panel.js
var _rebuildGen = /* @__PURE__ */ new WeakMap();
function setupPowerPromptPanel(node) {
  ...
  const isYamlConnected = () => (node.inputs?.find((i) => i.name === "yaml_input")?.link ?? null) != null;
...
async function rebuildVarControls(varsSection, yamlText, currentVars, node) {
  ...
  if (_rebuildGen.get(node) !== gen) return;
  const controls = def?.error || !def ? [] : def.controls.filter((c) => !c.hidden);
  if (!def || def.error) {
  ...
  for (const item of ordered) {
    ...
    const existingIsLarge = existing ? existing.querySelector(".pp-chip-select") != null || existing.querySelector(".pp-search-select") != null : false;
    const canUpdate = existing && existing.dataset.varWidget === ctrl.widget && existing.dataset.varIsAny === String(ctrl.is_any) && !isLarge && !existingIsLarge;
    if (canUpdate) {
      if (ctrl.widget === "dropdown") {
...
function setupPartialPanel(node) {
  ...
  node.addDOMWidget("_partial_panel", "ppPartialPanel", panel, {
    ...
    getMinHeight() {
      return isYamlConnected() ? 0 : YAML_MIN_HEIGHT;
    },
    hideOnZoom: false
  });
  const origOnConnectionsChange = node.onConnectionsChange?.bind(node);
